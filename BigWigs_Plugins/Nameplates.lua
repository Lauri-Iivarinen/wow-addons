...
local function getIconFrame()
	...
	if next(iconFrameCache) then
		...
		iconFrame = CreateFrame("Frame", "BigWigsNameplateIconFrame"..iconFrameCount, UIParent)
		iconFrame:SetPoint("CENTER")
		iconFrame:SetIgnoreParentScale(true)
		iconFrame:SetFrameStrata(db.iconFrameStrata) -- "MEDIUM"
		iconFrame:SetFixedFrameStrata(true)
		iconFrame:SetFrameLevel(db.iconFrameLevel) -- 5500
						...
						args = {
							...
							iconDisabled = {
								...
								confirm = function(_, value)
									if value then
										return L.disableDesc:format("Nameplate Icons")
									end
								end,
							},
							resetHeader = {
			...
			nameplateTextSettings = {
				...
				args = {
					...
					textDisabled = {
						...
						confirm = function(_, value)
							if value then
								return L.disableDesc:format("Nameplate Text")
							end
						end,
					},
					resetHeader = {
...
local function createNameplateIcon(module, guid, key, length, icon)
	...
	if guid == target then
		...
		iconFrame:SetFrameLevel(db.iconFrameLevel + 55) -- 5555
	else
		iconFrame:SetSize(db.iconWidthOthers, db.iconHeightOthers)
		iconFrame:SetFrameLevel(db.iconFrameLevel) -- 5500
	end
...
function plugin:StartNameplate(_, module, guid, key, length, customIconOrText)
	...
	if not customIconOrText or type(customIconOrText) == "number" then
		if db.iconDisabled then return end
	...
	elseif type(customIconOrText) == "string" then
		if db.textDisabled then return end
		...
		if nameplateIcons[guid] then
			for _, tbl in next, nameplateIcons[guid] do
				if tbl.nameplateFrame then
					...
					tbl.nameplateFrame:SetFrameLevel(db.iconFrameLevel + 55) -- 5555
				end
			end
			rearrangeNameplateIcons(guid)
		end
		if prevTarget and nameplateIcons[prevTarget] then
			for _, tbl in next, nameplateIcons[prevTarget] do
				if tbl.nameplateFrame then
					...
					tbl.nameplateFrame:SetFrameLevel(db.iconFrameLevel) -- 5500
				end
			end
