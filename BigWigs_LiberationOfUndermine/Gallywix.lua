...
--[[
Phases                   MATCHED type="removebuff" and ability.id=1220290 IN (1) END or (ability.id IN (469293,1214369) and type="removebuff") or (ability.id=1222831 and type="begincast")
TotalDestruction         ability.id=1214369 and type="removebuff"   //    type="interrupt" and stoppedAbility.id=1214369
OverloadedCoils          ability.id=1222831 and type="begincast"

Plating                  ability.id=1214229 and type="applydebuff"
GigaBlast                ability.id=469327 and type="begincast"
CircuitReboot            ability.id=1226891 and type IN ("applybuff","removebuff")
MayhemRockets            ability.id=1218696 and type="begincast"

GigaCoils                ability.id=469293 and type IN ("applybuff","removebuff")
VentingHeat              ability.id=466751 and type="begincast"
Suppression              ability.id=467182 and type="begincast"
    GallybuxFinaleBlast      ability.id=1219333 and type="begincast"
BigBombs                 ability.id IN (465952,1214607,1218546) and type="begincast"
    BadDelatedBoom           ability.id=466153 and type="damage"
    OverloadedRockets        ability.id=1214755 and type="applydebuff"
Canisters                ability.id IN (466340,466341,466342,1217987,1218488) and type="begincast"
    FusedCanisters           ability.id=466344 and type="applydebuff"
    TickTockCanisters         ability.id=466342 and type="cast"
EgoCheck                 ability.id=466958 and type="begincast"

ShockBarrage             ability.id=466834 and type="begincast"
Wrench                   ability.id=1216845 and type="begincast"
...
local timersMythic = {
	{ -- Phase 1
		...
		[466958] = { 16.0, 21.6, 14.5, 20.0, 17.1, 12.4, 16.5, 23.7, 11.5, 23.8, 17.1, 0 }, -- Ego Check
...
local function cd(spellId, count)
	-- if mod:Story() then
	-- 	return timersStory[stage][spellId][count]
	-- end
...
if L then
	...
	L.big_bad_buncha_bombs = "Big Bombs"
	L.bigger_badder_bomb_blast = "Tank Circle"
	L.overloaded_rockets = "Blaze"
	L.time_release_crackle = "Crackle"
	L.giga_bomb_detonation = "Speed Boost"
	L.total_destruction = "DESTRUCTION!"
	L.giga_blast = "Frontal"

	L.combination_canisters = "Soaks"
	L.biggest_baddest_bomb_barrage = "Spread Circles"
	L.scatterbomb_canisters = "Group Soak"

	L.duds = "Duds" -- Short for 1500-Pound "Dud"
	L.duds_explode = "Duds Explode"
...
function mod:GetOptions()
	return {
		...
		-- Stage One: The House of Chrome
			{466340, "CASTBAR"}, -- Scatterblast Canisters
		...
		-- Stage Two: Mechanical Maniac
			...
			469362, -- Charged Giga Bomb
				...
				469795, -- Giga Bomb Detonation
					-- 1215209, -- Sabotage Zone (Damage)
					-- 1220846, -- Control Meltdown

			{466341, "CASTBAR", "PRIVATE"}, -- Fused Canisters
		...
		-- Stage Three: What an Arsenal!
			...
			{466342, "CASTBAR"}, -- Tick-Tock Canisters
		...
		[465952] = L.big_bad_buncha_bombs,
		[1217292] = L.time_release_crackle,
		[1214229] = CL.shield, -- Armageddon-class Plating
		[466340] = L.scatterblast_canisters,
		[469795] = L.giga_bomb_detonation,
		[1214369] = L.total_destruction,
		[466341] = L.fused_canisters,
		[1214607] = L.bigger_badder_bomb_blast,
		[466342] = L.tick_tock_canisters,
		[1214755] = L.overloaded_rockets,
		[469327] = L.giga_blast,

		[1217987] = L.combination_canisters,
		[1218488] = L.scatterbomb_canisters,
		[474130] = CL.heal_absorb, -- Mechmastermind's Canisters
		[1218546] = L.biggest_baddest_bomb_barrage,
	}
end
...
function mod:OnEngage()
	...
	elseif not self:Mythic() then
		if not self:LFR() then
			self:Bar(466340, cd(466340, 1), CL.count:format(self:SpellName(466340), fullCanistersCount)) -- Scatterblast Canisters
		end
		self:Bar(465952, cd(465952, 1) - 4.5, CL.count:format(self:SpellName(465952), fullBombsCount)) -- Big Bad Buncha Bombs (emote is 2.2s earlier, debuffs 4.5s)
		self:Bar(467182, cd(467182, 1), CL.count:format(self:SpellName(467182), fullSuppressionCount)) -- Suppression
		self:Bar(466751, cd(466751, 1), CL.count:format(self:SpellName(466751), fullVentingHeatCount)) -- Venting Heat
...
function mod:CHAT_MSG_MONSTER_YELL(_, msg)
	if msg:find(L.story_phase_trigger, nil, true) then
		...
		self:Debug("Boss Health", self:GetHealth("boss1"))
...
function mod:CHAT_MSG_RAID_BOSS_EMOTE(_, msg)
	...
	if msg:find("spell:465952", nil, true) then
		...
		self:StopBar(CL.count:format(self:SpellName(465952), fullBombsCount))
		self:Message(465952, "red", CL.count:format(self:SpellName(465952), fullBombsCount))
		self:Debug("Boss Power", UnitPower("boss1"))
		...
		if not self:Story() then
			...
			if bombsCD and bombsCD > 0 then
				self:CDBar(465952, bombsCD - 2.3, CL.count:format(self:SpellName(465952), fullBombsCount))
			end
	...
	elseif self:Mythic() and msg:find("spell:1218546", nil, true) then
		...
		self:StopBar(CL.count:format(self:SpellName(1218546), fullBombsCount))
		self:Message(1218546, "red", CL.count:format(self:SpellName(1218546), fullBombsCount))
		self:PlaySound(1218546, "alarm") -- avoid
		bombsCount = bombsCount + 1
		fullBombsCount = fullBombsCount + 1
		local bombsCD = cd(1218546, bombsCount)
		if bombsCD and bombsCD > 0 then
			self:Bar(1218546, bombsCD - 2.3, CL.count:format(self:SpellName(1218546), fullBombsCount))
		end
...
-- 	self:StopBar(CL.count:format(args.spellName, fullBombsCount))
-- 	self:Message(args.spellId, "red", CL.count:format(args.spellName, fullBombsCount))
-- 	self:PlaySound(args.spellId, "alert")
-- 	bombsCount = bombsCount + 1
-- 	fullBombsCount = fullBombsCount + 1
-- 	self:CDBar(args.spellId, cd(465952, bombsCount), CL.count:format(args.spellName, fullBombsCount))
...
do
	...
	function mod:FifteenHundredPoundDudApplied(args)
		...
		if args.time - prev > 2 then
			self:Bar(args.spellId, 15, L.duds_explode)
		end
	end

	function mod:FifteenHundredPoundDudRemoved(args)
		self:StopBar(L.duds_explode)
		spawnedDuds = spawnedDuds - 1
...
function mod:FocusedDetonationApplied(args)
	...
	self:TargetBar(1217292, 10, args.destName) -- Time-Release Crackle
end
...
function mod:ScatterblastCanisters(args)
	self:StopBar(CL.count:format(args.spellName, fullCanistersCount))
	self:Message(args.spellId, "orange", CL.count:format(args.spellName, fullCanistersCount))
	...
	self:CDBar(args.spellId, cd(args.spellId, canistersCount), CL.count:format(args.spellName, fullCanistersCount))
end
...
function mod:TrickShotsRemoved()
	...
	if self:GetStage() == 1 then
		...
		self:StopBar(CL.count:format(self:SpellName(466340), fullCanistersCount)) -- Scatterblast Canisters
		self:StopBar(CL.count:format(self:SpellName(465952), fullBombsCount)) -- Big Bad Buncha Bombs
		self:StopBar(CL.count:format(self:SpellName(467182), fullSuppressionCount)) -- Suppression
		self:StopBar(CL.count:format(self:SpellName(466751), fullVentingHeatCount)) -- Venting Heat
...
function mod:UNIT_SPELLCAST_START(_, unit, _, spellId)
	if spellId == 469286 then -- Giga Coils
		...
		if not self:Mythic() then
			self:StopBar(CL.count:format(self:SpellName(466340), fullCanistersCount)) -- Scatterblast Canisters
			self:StopBar(CL.count:format(self:SpellName(466341), fullCanistersCount)) -- Fused Canisters
			self:StopBar(CL.count:format(self:SpellName(466342), fullCanistersCount)) -- Tick-Tock Canisters
			self:StopBar(CL.count:format(self:SpellName(465952), fullBombsCount)) -- Big Bad Buncha Bombs
...
function mod:GigaCoilsRemoved()
	...
	if stage == 2 then
		if not self:LFR() then
			self:CDBar(466341, cd(466341, canistersCount), CL.count:format(self:SpellName(466341), fullCanistersCount)) -- Fused Canisters
		end
		local bombsCD = cd(465952, bombsCount)
		if bombsCD and bombsCD > 0 then
			self:CDBar(465952, bombsCD - 4.5, CL.count:format(self:SpellName(465952), fullBombsCount)) -- Big Bad Buncha Bombs
		end
	elseif stage == 3 then
		...
		if not self:LFR() then
			self:CDBar(466342, cd(466342, canistersCount), CL.count:format(self:SpellName(466342), fullCanistersCount)) -- Tick-Tock Canisters
		end
		self:CDBar(1214607, cd(1214607, bombsCount), CL.count:format(self:SpellName(1214607), fullBombsCount)) -- Bigger Badder Bomb Blast
	end
...
do
	...
	function mod:ArmageddonClassPlatingApplied(args)
		...
		self:CDBar(1214369, self:Mythic() and 8.6 or self:LFR() and 10.6 or 9.6) -- TOTAL DESTRUCTION!!!
	end
...
function mod:BiggerBadderBombBlast(args)
	...
	if not self:Story() then
		self:Bar(args.spellId, cd(args.spellId, bombsCount), CL.count:format(args.spellName, fullBombsCount))
...
function mod:TickTockCanisters(args)
	...
	canistersCount = canistersCount + 1
	fullCanistersCount = fullCanistersCount + 1
	self:Bar(args.spellId, cd(args.spellId, canistersCount), CL.count:format(args.spellName, fullCanistersCount))
end
...
function mod:CombinationCanisters(args)
	...
	self:Message(args.spellId, "orange", CL.count:format(args.spellName, fullCanistersCount))
	self:PlaySound(args.spellId, "alert") -- soak
	-- self:Bar(466341, 7) -- Fused Canisters
	-- self:Bar(466342, 8) -- Tick-Tock Canisters
	canistersCount = canistersCount + 1
	fullCanistersCount = fullCanistersCount + 1
	self:Bar(args.spellId, cd(args.spellId, canistersCount), CL.count:format(args.spellName, fullCanistersCount))
end
