...
do
	...
	local releaseType
	local myGitHash = "LIQUID" -- The ZIP packager will replace this with the Git hash.
	local releaseString
	--@alpha@
...
do
	...
	-- C_SeasonInfo.GetCurrentDisplaySeasonID() -- XXX returns 0 on cold load
	for _, id in next, public.isTestBuild and testingSeason or currentSeason do
		local header = public.zoneTbl[id]
		public.zoneTbl[id] = { header, lw_cs }
	end
end
...
do
	...
	for i = 1, GetNumAddOns() do
		...
		if GetAddOnEnableState(name, myGUID) == 2 then -- if addonState ~= "DISABLED" then (only works when disabled on ALL characters)
			...
			if minVersion and GetAddOnDependencies(i) == "BigWigs" and not guildDisableContentWarnings then -- Safety
...
do
	...
	for i = 1, GetNumAddOns() do
		...
		if addonToCheck and not guildDisableContentWarnings then
