...
do
	local menuArt = {
		...
		CASTBAR = "Interface\\AddOns\\BigWigs\\Media\\Icons\\Menus\\CastBar",
...
do
	local function formatName(name)
		return LiquidAPI and LiquidAPI:GetName(name) or gsub(name, "%-.+", "*") -- Replace server names with *
	end
	...
	do
		local _, class = UnitClass("player")
		myNameWithColor = hexColors[class] .. myName .. "|r"
	end
...
--- Get the mob/npc spawn index from a GUID.
-- @string guid GUID of a mob/npc
-- @return mob/npc spawn index
function boss:MobSpawnIndex(guid)
	if not guid then return end
	local _, _, _, _, _, _, uid = strsplit("-", guid)
	local index = bit.rshift(band(tonumber(string.sub(uid, 1, 5), 16), 0xffff8), 3) + 1
	return index
end

--- Get the mob/npc spawn time from a GUID.
-- @string guid GUID of a mob/npc
-- @return mob/npc spawn time
function boss:MobSpawnTime(guid)
	if not guid then return end
	local _, _, _, _, _, _, uid = strsplit("-", guid)
	local spawnEpochOffset = bit.band(tonumber(string.sub(uid, 5), 16), 0x7fffff)
	...
	local function printTargets(self, key, playerTable, color, text, icon)
		...
		if playersInTable > 0 and (not playerTable.prevPlayersInTable or playerTable.prevPlayersInTable < playersInTable) then
			...
			if textType == "table" then
				msg = type(text[1]) == "string" and text[1] or spells[text[1] or key]
				msgYou = type(text[2]) == "string" and text[2] or spells[text[2] or key]
			else
			...
			if playersInTable-previousAmount == 1 and playerTable[playersInTable] == myName then
				...
				if not meEmphasized then -- We already did a ME_ONLY_EMPHASIZE print in :TargetsMessage
					...
					if marker then
						self:SendMessage("BigWigs_Message", self, key, format(L.you_icon, msgYou, marker), "blue", texture, emphasized)
					else
						self:SendMessage("BigWigs_Message", self, key, format(L.you, msgYou), "blue", texture, emphasized)
					end
				end
			else
		...
		if band(self.db.profile[key], C.ME_ONLY) == C.ME_ONLY then -- We allow ME_ONLY even if MESSAGE off
			if playerTable[playersInTable] == myName and checkFlag(self, key, C.ME_ONLY) then -- Use checkFlag for the role check
				...
				if textType == "table" then
					...
					msgYou = textType == "string" and text or spells[text or key]
				end
				local texture = icon ~= false and icons[icon or key]
				local marker = playerTable[myName]
				if marker then
					self:SendMessage("BigWigs_Message", self, key, format(L.you_icon, msgYou, marker), "blue", texture, isEmphasized)
				else
					self:SendMessage("BigWigs_Message", self, key, format(L.you, msgYou), "blue", texture, isEmphasized)
				end
			end
		elseif checkFlag(self, key, C.MESSAGE) then
			if playerTable[playersInTable] == myName and band(self.db.profile[key], C.ME_ONLY_EMPHASIZE) == C.ME_ONLY_EMPHASIZE then
				...
				local marker = playerTable[myName]
				if marker then
					self:SendMessage("BigWigs_Message", self, key, format(L.you_icon, msgYou, marker), "blue", texture, true)
				else
					self:SendMessage("BigWigs_Message", self, key, format(L.you, msgYou), "blue", texture, true)
				end
			end
