...
function LRP:Modernize()
        ...
        for _, difficultyReminders in pairs(LiquidRemindersSaved.reminders) do
            for _, encounterReminders in pairs(difficultyReminders) do
                for _, reminderData in pairs(encounterReminders) do
                    if reminderData.trigger and reminderData.trigger.relativeTo and reminderData.trigger.relativeTo.spellID then
    ...
    if internalVersion < 10 then
        for _, difficultyReminders in pairs(LiquidRemindersSaved.reminders) do
            for _, encounterReminders in pairs(difficultyReminders) do
                for _, reminderData in pairs(encounterReminders) do
                    if reminderData.trigger and not reminderData.countdown then
    ...
    -- Mug'Zee was using Head Honcho buffs as phase changes, but the very first one doesn't always happen on pull
    -- Change to their Uncontrolled Destruction/Elemental Carnage buffs instead
    if internalVersion < 22 then
        if not LiquidRemindersSaved.reminders[3015] then return end

        for _, difficultyReminders in pairs(LiquidRemindersSaved.reminders[3015]) do
            for _, profileReminders in pairs(difficultyReminders) do
                for _, reminderData in pairs(profileReminders) do
                    local relativeTo = reminderData.trigger.relativeTo
                    ...
                    if reminderData.trigger.relativeTo then
                        ...
                        if event == "SPELL_AURA_APPLIED" then
                            if value == 466460 then -- Head Honcho: Zee
                                relativeTo.value = 468694 -- Uncontrolled Destruction
                            elseif value == 466459 then -- Head Honcho: Mug
                                relativeTo.value = 468658 -- Elemental Carnage
                            end
                        end
                    end
                end
    ...
    if internalVersion < 23 then
        for _, encounterReminders in pairs(LiquidRemindersSaved.reminders) do
            for _, difficultyReminders in pairs(encounterReminders) do
                for _, profileReminders in pairs(difficultyReminders) do
                    for i, reminderData in pairs(profileReminders) do
                        if type(reminderData) ~= "table" then
                            profileReminders[i] = nil
                        end
                    end
                end
            end
